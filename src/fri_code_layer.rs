...
pub fn generate_enlarged_evaluation_domain(domain_size: usize) -> Vec<FieldElement> {
    ...
    // For quadratic symmetry, we need the domain such that domain[i]^2 = domain[i + domain_size/2]^2
    // This means domain[i + domain_size/2] = ±domain[i]
    // Let's construct the domain directly to ensure this property
    
    // Generate the first half of the domain using powers of a coset offset
    let half_size = domain_size / 2;
    let coset_offset = g; // Start with the generator itself
    
    let mut domain = Vec::with_capacity(domain_size);
    
    // First half: g * coset_offset^i for i = 0..half_size
    for i in 0..half_size {
        domain.push(g * coset_offset.pow(&[i as u64]));
    }
    
    // Second half: -domain[i] for i = 0..half_size to ensure quadratic symmetry
    for i in 0..half_size {
...
mod tests {
    ...
    fn test_eval_domain_symetry() {
        ...
        // Debug: Let's investigate the coset offset properties
        let g = FieldElement::MULTIPLICATIVE_GENERATOR;
        let coset_offset = g.pow(&[4u64]); // Updated to match the new implementation
        let minus_one = FieldElement::from(0u64) - FieldElement::from(1u64);
        
        println!("Domain size: {}", domain_size);
        println!("Generator g: {:?}", g);
        println!("Coset offset (g^4): {:?}", coset_offset);
        println!("-1 in field: {:?}", minus_one);
        println!("Coset offset^(domain_size/2): {:?}", coset_offset.pow(&[(domain_size / 2) as u64]));
        println!("Is coset_offset^(domain_size/2) == -1? {}", coset_offset.pow(&[(domain_size / 2) as u64]) == minus_one);
        
        println!("domain[100]: {:?}", domain[100]);
        println!("domain[{}]: {:?}", half_domain_size + 100, domain[half_domain_size + 100]);
        println!("domain[100]^2: {:?}", domain[100].pow(&[2u64]));
        println!("domain[{}]^2: {:?}", half_domain_size + 100, domain[half_domain_size + 100].pow(&[2u64]));

        assert_eq!(
            domain[100].pow(&[2u64]),
            domain[half_domain_size + 100].pow(&[2u64]),
            "Symmetry should be respected"
